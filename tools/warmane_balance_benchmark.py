...
TOP_GEARED_BAND_PERCENTILE = 0.90
...
def apply_runtime_constants(tables: dict[str, Any]) -> None:
    ...
    GS_GS2_STAT_SCALE = float(tables["GS_GS2_STAT_SCALE"])
    GS_GEM_SCALE = float(tables["GS_GEM_SCALE"])
    GS_ENCHANT_SCALE = float(tables["GS_ENCHANT_SCALE"])
    GS_PVE_RESILIENCE_RATE = float(tables["GS_PVE_RESILIENCE_RATE"])
    GS_PVP_RESILIENCE_RATE = float(tables["GS_PVP_RESILIENCE_RATE"])
    GS_PVE_RESILIENCE_FLOOR = float(tables["GS_PVE_RESILIENCE_FLOOR"])
    GS_PVP_RESILIENCE_CAP = float(tables["GS_PVP_RESILIENCE_CAP"])
    GS_CAP_BONUS_ANCHOR_LOW_GS2 = float(tables["GS_CAP_BONUS_ANCHOR_LOW_GS2"])
    GS_CAP_BONUS_ANCHOR_HIGH_GS2 = float(tables["GS_CAP_BONUS_ANCHOR_HIGH_GS2"])
    GS_CAP_BONUS_ANCHOR_LOW_BONUS = float(tables["GS_CAP_BONUS_ANCHOR_LOW_BONUS"])
    GS_CAP_BONUS_ANCHOR_HIGH_BONUS = float(tables["GS_CAP_BONUS_ANCHOR_HIGH_BONUS"])
    GS_CAP_BONUS_MIN = float(tables["GS_CAP_BONUS_MIN"])
    GS_CAP_BONUS_MAX = float(tables["GS_CAP_BONUS_MAX"])
...
def normalize_spec_name(name: str, class_name: str) -> str:
    ...
    if class_name.upper() == "MAGE" and key == "FROST":
        return "MAGE_FROST"
    if class_name.upper() == "PRIEST" and key == "HOLY":
        return "PRIEST_HOLY"
    if class_name.upper() == "DRUID" and key == "RESTORATION":
...
def get_class_spec_candidates(class_token: str, tables: dict[str, Any]) -> list[str]:
    ...
    for spec_key in tables["GS_ClassSpecOrder"].get(class_token) or []:
        if class_token == "DRUID" and spec_key == "FERAL":
...
def is_item_compatible(item: dict[str, Any], class_token: str, profile: dict[str, Any]) -> bool:
    ...
    ignore_armor_downgrade = class_token == "DRUID" and profile.get("role") in {"CASTER", "HEALER"}
    ...
    if role in {"CASTER", "HEALER"} and stats.get("STR", 0) > 0 and stats.get("SP", 0) == 0 and stats.get("INT", 0) == 0:
        return False
    if role in {"MELEE", "RANGED"} and stats.get("SP", 0) > 0 and all(stats.get(key, 0) == 0 for key in ("STR", "AGI", "AP", "RAP")):
        return False
    ...
    if role in {"CASTER", "HEALER"} and role_signature in {"MELEE", "RANGED"} and stats.get("SP", 0) == 0 and stats.get("INT", 0) == 0:
        return False
    if role in {"MELEE", "RANGED"} and role_signature in {"CASTER", "HEALER"} and all(stats.get(key, 0) == 0 for key in ("STR", "AGI", "AP", "RAP")):
        return False
    return True
...
def score_item_with_debug(item: dict[str, Any], class_token: str, spec_key: str, tables: dict[str, Any]) -> tuple[int, int, list[str], dict[str, Any]]:
    ...
    if not is_item_compatible(item, class_token, profile):
        return 0, 0, ["incompatible-item"], debug
...
def is_rogue_poison_cap_spec(spec_key: str) -> bool:
    return spec_key in {"ASSASSINATION", "COMBAT", "SUBTLETY"}
...
def apply_character_caps(spec_key: str, items: list[dict[str, Any]], pre_cap_gs2: int, tables: dict[str, Any], race_token: str | None = None) -> tuple[int, dict[str, Any] | None, dict[str, float]]:
    cap_profile = tables["GS_CapProfiles"].get(spec_key)
    profile = tables["GS_SpecProfiles"].get(spec_key)
...
def get_snapshot_spec_diagnostics(items: list[dict[str, Any]], class_token: str, spec_key: str, tables: dict[str, Any], race_token: str | None = None) -> dict[str, Any] | None:
    ...
    cap_bonus, _, _ = apply_character_caps(spec_key, items, total_before_caps, tables, race_token)
    return {
        "spec_key": spec_key,
        "role": profile.get("role"),
        ...
        "cap_bonus": cap_bonus,
        "total": total_before_caps + cap_bonus,
...
def build_character_record(profile: dict[str, Any], fetcher: Fetcher, tables: dict[str, Any], item_cache: dict[tuple[int, int], dict[str, Any]], gem_cache: dict[int, ResolvedGem]) -> dict[str, Any]:
    prepared_items = []
    enriched_items = []
    runtime_items = []
    legacy = 0
    ...
    cap_bonus, cap_breakdown, cap_stats = apply_character_caps(resolved_spec, runtime_items, gs2_pre_cap, tables, profile.get("raceToken"))
    active_spec_gs2 = math.floor(gs2_pre_cap + cap_bonus)
    ...
    best_alternate_delta_pct = 0.0
    off_spec_flag = False
    off_spec_reason = None
    if best_alternate_gs2 is not None and active_spec_gs2 > 0:
        best_alternate_delta_pct = round(((best_alternate_gs2 - active_spec_gs2) / active_spec_gs2) * 100.0, 2)
        if best_alternate_gs2 > active_spec_gs2 * (1 + GS_OFFSPEC_MIN_RATIO):
            off_spec_flag = True
            off_spec_reason = get_offspec_reason(active_diagnostics, best_alternate_diagnostics)
    ...
    return {
        ...
        "gs2_final": active_spec_gs2,
        "active_spec_gs2": active_spec_gs2,
        "best_alternate_spec": best_alternate_spec,
        "best_alternate_spec_gs2": best_alternate_gs2,
        "best_alternate_delta_pct": best_alternate_delta_pct,
        "off_spec_flag": off_spec_flag,
        "off_spec_reason": off_spec_reason,
...
def summarize_records(records: list[dict[str, Any]], group_field: str) -> dict[str, Any]:
    groups: dict[str, list[dict[str, Any]]] = {}
    for record in records:
        groups.setdefault(str(record[group_field]), []).append(record)
    return {f"{group_field}:{name}": summarize_group(group_records) for name, group_records in groups.items()}
...
def build_balance_slice(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    top_band_threshold = percentile([record["avg_item_level"] for record in records], TOP_GEARED_BAND_PERCENTILE)
    return {
        "size": len(records),
        "median_gs2": round(median([record["gs2_final"] for record in records]), 2),
        "min_gs2": min(record["gs2_final"] for record in records),
        "max_gs2": max(record["gs2_final"] for record in records),
        "spread_pct": round(
            pct_delta(max(record["gs2_final"] for record in records), median([record["gs2_final"] for record in records]))
            - pct_delta(min(record["gs2_final"] for record in records), median([record["gs2_final"] for record in records])),
            2,
        ),
        "top_band_item_level_threshold": top_band_threshold,
        "role_groups": safe_json_value(summarize_records(records, "role")),
        "spec_groups": safe_json_value(summarize_records(records, "active_spec")),
    }
...
def annotate_records(records: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for record in records:
        spec_groups.setdefault(record["active_spec"], []).append(record)
        role_groups.setdefault(record["role"], []).append(record)

    groups_summary: dict[str, Any] = {}
    outliers: list[str] = []
    for record in records:
        spec_group = spec_groups[record["active_spec"]]
        group_key = f"spec:{record['active_spec']}" if len(spec_group) >= 2 else f"role:{record['role']}"
        ...
        top_geared_band_flag = record["avg_item_level"] >= top_band_threshold
        clean_record_flag = bool(diagnostic_summary["clean_record_flag"])
        top_band_clean_flag = top_geared_band_flag and clean_record_flag
        if outlier_flag:
            outliers.append(record["name"])

        record.update(
            {
                "global_delta_from_median_pct": round(global_delta, 2),
                "group_key": group_key,
                "group_median_gs2": round(group_median, 2),
                "group_delta_from_median_pct": round(group_delta, 2),
                "top_geared_band_flag": top_geared_band_flag,
                "top_band_clean_flag": top_band_clean_flag,
    ...
    for group_name, group_records in spec_groups.items():
        groups_summary[f"spec:{group_name}"] = summarize_group(group_records)
    for group_name, group_records in role_groups.items():
        groups_summary[f"role:{group_name}"] = summarize_group(group_records)
    ...
    clean_top_band_records = [record for record in records if record["top_band_clean_flag"]]
    summary = {
        ...
        "quality": {
            ...
            "clean_top_band_count": len(clean_top_band_records),
        ...
        "balance_views": {
            ...
            "clean_top_band": safe_json_value(build_balance_slice(clean_top_band_records)),
        },
        "groups": safe_json_value(groups_summary),
        "outliers": sorted(dict.fromkeys(outliers)),
    }
...
def export_csv(records: list[dict[str, Any]], path: Path) -> None:
    ...
    fieldnames = [
        ...
        "active_spec_gs2", "best_alternate_spec", "best_alternate_spec_gs2",
        "best_alternate_delta_pct", "off_spec_flag", "off_spec_reason",
        "global_delta_from_median_pct", "group_key", "group_median_gs2",
        "group_delta_from_median_pct", "top_geared_band_flag", "top_band_clean_flag",
...
def export_json(records: list[dict[str, Any]], summary: dict[str, Any], dataset_name: str, output_path: Path) -> None:
    ...
    payload = {
        ...
        "scoring_version": "gearscore2-benchmark-v4-runtime-parity",
        "assumptions": [
            "Active spec from Warmane profile is used as the benchmark spec.",
            "Offline benchmark does not model live aura buffs or target debuffs.",
            "Offline benchmark reproduces compatibility, cap, and off-spec comparison logic, but not live inspect transport or timing.",
            ...
            "CSV is a flattened summary; JSON is the full audit record.",
        ],
        "data_sources": {
            "snapshot": "warmane",
            "item_stats": "wotlkdb",
        ...
        "characters": safe_json_value(sorted(records, key=lambda entry: (-entry["gs2_final"], entry["name"]))),
    }
...
def main(argv: list[str]) -> int:
    ...
    for index, ref in enumerate(refs, start=1):
        ...
        records.append(build_character_record(profile, fetcher, tables, item_cache, gem_cache))
